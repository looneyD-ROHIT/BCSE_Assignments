	   LDA 204F	// store value of N in accumulator
	   MOV C,A	// copy N in register C to use as counter
	   LXI H,2050	// set up HL to point to address of first byte
	   LXI D,2100	// set up DL to point to address 2100H to store the even numbers
	   MVI B,00	// clear register B to count no of even values

STARTEVEN:	   MOV A,M	// place contents of HL in accumulator
	   ANI 01	// check if the last bit is 1
...
NTEVEN:	   INX H	// increment HL to point to next address
	   ...
	   STA 2300	// store the count value in 2300H
	   LDA 204F	// store value of N in accumulator
	   MOV C,A	// copy N in register C to use as counter
	   LXI H,2050	// set up HL to point to address of first byte
	   LXI D,2200	// set up DL to point to address 2200H to store the odd numbers
	   MVI B,00	// clear register B to count no of odd values
...
NTODD:	   INX H	// increment HL to point to next address
	   ...
	   STA 2301	// store the count value in 2301H
	   HLT	// stop
