	   ...
	   LDA 204E	// store contents of 204EH in accumulator
	   MOV B,A	// copy value of the number in register B
	   LXI H,2050	// set up HL to point to the location of first byte
	   MVI D,00	// clear register D to store position of number, if found
...
LOOP:	   MOV A,M	// place the byte specified by content of HL in accumulator
	   ...
	   STA 204D	// store the position in 204DH
	   HLT	// stop
...
NEXT:	   INX H	// increment HL to point to next address
	   ...
	   STA 204D	// store contents of accumulator in 204DH
	   HLT	// stop
