...
typedef struct reqmem{
    sem_t rentry, wentry, write, mut1, mut2;
...
int main(int argc, char** argv){
    ...
    R = atoi(argv[1]);
    W = atoi(argv[2]);
    required_memory *shared = (required_memory*) mmap(NULL, sizeof(required_memory), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
    ...
    __reader:{
	    ...
	    while (l--)	{
            ...
            // read can be done by multiple readers so, signalling rentry
