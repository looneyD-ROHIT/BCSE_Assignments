...
class Channel():
    ...
    def process(self):
        ...
        for i in range(maxlen):
            ...
            while choice == 'y':
                stnum, renum = input(
                    'Enter the sender and receiver station number: ').split()
                stnum = int(stnum)
                renum = int(renum)
                if stnum > self.totalstation or stnum <= 0 or renum > self.totalstation or renum <= 0:
                    ...
                    print(
                        'Multiplying final data with Code bits of sender station', stnum)
                    ...
                    print()
                    choice = input(
                        'Does any station want to receive data ? (y/n) ')
