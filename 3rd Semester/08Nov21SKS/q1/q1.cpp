/*
1.	In a library, for each book book-id, serial number (denotes copy number of 
a book), title, author, publisher and price are stored. Book-id and serial 
number together will be unique identifier for a book. Members are either student 
or faculty. Each member has unique member-id. Name, e-mail, address are also to 
...
(only for book return). While processing a transaction, check the validity of 
the member. While issuing, availability of a copy of the book is to be checked. 
While returning a book, it is to be checked whether this copy was issued to the 
member or not. A student member can have 2 books issued at a point of time. For 
faculty members it is 10. Transaction information is to be stored like date of 
transaction, member-id, book-id, serial number, returned or not. An entry is 
made when book is issued and updated when the book is returned. For storing the 
information consider files.
...
// overloading stream exttaction operator
...
// student class inheriting member class
...
// faculty class inheriting member class
...
// check to know if book issued to member or not
...
// check to know if book issued to member with a specific serial no.
...
// check to return if transaction matches
...
// function to get no of issued books
...
// function to know if book is available or not
...
// function to check issue eligibility
